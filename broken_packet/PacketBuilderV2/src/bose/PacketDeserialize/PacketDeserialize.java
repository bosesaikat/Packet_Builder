...
import static bose.BrokenPacket.BrokenPacketBuilder.BROKEN_PACKET_HEADER_SIZE;
...
public class PacketDeserialize {
    ...
    public  void deSerialize(ArrayList<byte[]> packet){
        ...
        for(byte[] b : packet){
          ...
          //  System.out.println("index after header " + index );
            ...
            int sequenceNoCode = byteToDataConverter.getInteger(b, index, 2);
            index += 2;
          ...
          //  System.out.println("Seq no : " + sequenceNo );
            ...
            if( previousSequeceNo + 1 == sequenceNo ){
                ...
                int totalPacketCode = byteToDataConverter.getInteger(b, index, 2);
                index += 2;
                int totalPacketLength  = byteToDataConverter.getInteger(b, index, 1) ;
                index++;
                int totatPacketNo = byteToDataConverter.getInteger(b, index, totalPacketLength);
                index += totalPacketLength ;
                
                
                int currentPacketCode = byteToDataConverter.getInteger(b, index, 2);
                index += 2;
                int currentPacketLength  = byteToDataConverter.getInteger(b, index, 1) ;
                index++;
                int currentPacketNo = byteToDataConverter.getInteger(b, index, currentPacketLength );
                index += currentPacketLength ;
                
                int remainingPacketCode = byteToDataConverter.getInteger(b, index, 2);
                index += 2;
                int remainingPacketLength  = byteToDataConverter.getInteger(b, index, 1) ;
                index++;
                int remainingPacketNo = byteToDataConverter.getInteger(b, index, remainingPacketLength );
                index += remainingPacketLength ;
                
                byte[] mergeData = new byte[ dataArrayLength + b.length - index ];
                
                if(dataArrayLength > 0)
                    System.arraycopy(dataArray, 0, mergeData, 0, dataArrayLength);
    ...
    private int headerDeSerialize( byte[] b ){
       ...
       //     System.out.print(b[i] + " ");
            ...
            while(index < BROKEN_PACKET_HEADER_SIZE ){
